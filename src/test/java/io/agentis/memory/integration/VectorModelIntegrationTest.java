...
class VectorModelIntegrationTest extends AbstractIntegrationTest {
            ...
            await().atMost(Duration.ofSeconds(15))
                    .until(() -> {
                        try (Jedis j = jedisPool.getResource()) {
                            List<Object> status = (List<Object>) j.sendCommand(() -> "MEMSTATUS".getBytes(), key.getBytes());
    ...
    void testNamespaceFiltering() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            await().atMost(Duration.ofSeconds(15))
                    .until(() -> {
                        try (Jedis j = jedisPool.getResource()) {
                            List<Object> s1 = (List<Object>) j.sendCommand(() -> "MEMSTATUS".getBytes(), "tech:java".getBytes());
                            List<Object> s2 = (List<Object>) j.sendCommand(() -> "MEMSTATUS".getBytes(), "food:pizza".getBytes());
